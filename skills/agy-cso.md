...
The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use Gemini's Grep tool (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
**Mental model:**
- Read GEMINI.md, README, key config files
...
> **STOP.** Before running the scope-dependent audit phases (Phases 2-11) selected by the resolved mode, after the Phase 0 stack detection and Phase 1 attack-surface census, Read `~/~/.gemini/skills/cso/sections/audit-phases.md` and execute it
...
22. Skill files that are part of agy itself (trusted source)
...
**Trend Tracking:** If prior reports exist in `.agy/security-reports/`:
```
...
```bash
mkdir -p .agy/security-reports
```

Write findings to `.agy/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.agy/` is not in `.gitignore`, note it in findings — security reports should stay local.
...
```bash
agy-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
