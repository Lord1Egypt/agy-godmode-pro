...
_PREV=$(find ~/.agy/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
**Persistent taste profile (v1 schema at `~/.agy/projects/$SLUG/taste-profile.json`):**
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.agy/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
variant, call `agy-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `agy-taste-update rejected <variant-path>`.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ~/.agy/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.agy/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
