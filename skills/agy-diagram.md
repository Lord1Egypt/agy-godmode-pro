...
(artifacts the user can commit), else `/tmp/agy-diagrams/`. Derive
...
so concurrent sessions and mixed agy versions never clobber each other:

```bash
BUNDLE=""
for c in "~/.gemini/skills/lib/diagram-render/dist/diagram-render.html" \
         "$(git rev-parse --show-toplevel 2>/dev/null)/lib/diagram-render/dist/diagram-render.html"; do
  [ -f "$c" ] && BUNDLE="$c" && break
done
[ -z "$BUNDLE" ] && echo "BUNDLE_MISSING — run: cd ~/.gemini/skills/agy && bun run build:diagram-render" && exit 1
SHA=$(shasum -a 256 "$BUNDLE" | cut -c1-16)
STAGED="/tmp/agy-diagram-render-$SHA.html"
