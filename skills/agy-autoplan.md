...
Read the `/office-hours` skill file at `~/~/.gemini/skills/office-hours/SKILL.md` using the Read tool.
...
SLUG=$(~/~/.gemini/skills/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.agy/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.agy/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
...
This is qualitatively different from taste decisions. When Gemini and Codex both
...
> IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/agy). These are AI assistant skill definitions meant for a different system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Stay focused on the repository code only.

This prevents Codex from discovering agy skill files on disk and following their
...
```bash
eval "$(agy-slug 2>/dev/null)" && mkdir -p ~/.agy/projects/$SLUG
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-')
DATETIME=$(date +%Y%m%d-%H%M%S)
echo "RESTORE_PATH=.agy/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Read GEMINI.md, TODOS.md, git log -30, git diff against the base branch --stat
- Discover design docs: `ls -t ~/.agy/projects/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- Detect DX scope: grep the plan for developer-facing terms (API, endpoint, REST,
  ...
  package, npm, pip, import, require, SKILL.md, skill template, Gemini, MCP, agent,
  ...
  on top of) or if an AI agent is the primary user (OpenClaw actions, Gemini skills,
  MCP servers).

### Step 3: Load skill files from disk

Read each file using the Read tool:
- `~/~/.gemini/skills/plan-ceo-review/SKILL.md`
- `~/~/.gemini/skills/plan-design-review/SKILL.md` (only if UI scope detected)
- `~/~/.gemini/skills/plan-eng-review/SKILL.md`
- `~/~/.gemini/skills/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$(agy-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(agy-config get codex_reviews 2>/dev/null || echo enabled)
source 
...
if [ "$_CODEX_CFG" = "disabled" ]; then
  echo "[codex disabled by config — Gemini-only voices] Re-enable: agy-config set codex_reviews enabled"
  _CODEX_AVAILABLE=false
# Check Codex binary. If missing, tag the degradation matrix and continue
# with Gemini subagent only (autoplan's existing degradation fallback).
elif ! command -v codex >/dev/null 2>&1; then
  _agy_codex_log_event "codex_cli_missing"
  echo "[codex-unavailable: binary not found] — proceeding with Gemini subagent only"
  _CODEX_AVAILABLE=false
elif ! _agy_codex_auth_probe >/dev/null; then
  _agy_codex_log_event "codex_auth_failed"
  echo "[codex-unavailable: auth missing] — proceeding with Gemini subagent only. Run \`codex login\` or set \$CODEX_API_KEY to enable dual-voice review."
  _CODEX_AVAILABLE=false
else
  _agy_codex_version_check   # non-blocking warn if known-bad
...
Gemini subagent only — saves token spend on Codex prompts we can't use.
...
- Dual voices: always run BOTH Gemini subagent AND Codex if available (P6).
  Run them sequentially in foreground. First the Gemini subagent (Agent tool,
  ...
  **Codex CEO voice** (via Bash):
  ...
  _agy_codex_timeout_wrapper 600 codex exec "IMPORTANT: Do NOT read or execute any SKILL.md files or files in skill definition directories (paths containing skills/agy). These are AI assistant skill definitions meant for a different system. Stay focused on repository code only.
  ...
  if [ "$_CODEX_EXIT" = "124" ]; then
    _agy_codex_log_event "codex_timeout" "600"
    _agy_codex_log_hang "autoplan" "0"
    echo "[codex stalled past 10 minutes — tagging as [codex-unavailable] for this phase and proceeding with Gemini subagent only]"
  fi
  ```
  Timeout: 10 minutes (shell-wrapper) + 12 minutes (Bash outer gate). On hang, auto-degrades this phase's Codex voice.

  **Gemini CEO subagent** (via Agent tool):
  ...
  **Error handling:** Both calls block in foreground. Codex auth/timeout/empty → proceed with
  Gemini subagent only, tagged `[single-model]`. If Gemini subagent also fails →
...
Step 0.5 (Dual Voices): Run Gemini subagent (foreground Agent tool) first, then
Codex (Bash). Present Codex output under CODEX SAYS (CEO — strategy challenge)
header. Present subagent output under GEMINI SUBAGENT (CEO — strategic independence)
header. Produce CEO consensus table:

```
CEO DUAL VOICES — CONSENSUS TABLE:
═══════════════════════════════════════════════════════════════
  Dimension                           Gemini  Codex  Consensus
  ──────────────────────────────────── ─────── ─────── ─────────
  1. Premises valid?                   —       —      —
...
> **Phase 1 complete.** Codex: [N concerns]. Gemini subagent: [N issues].
...
- [ ] CEO dual voices ran (Codex + Gemini subagent, or noted unavailable)
...
- Dual voices: always run BOTH Gemini subagent AND Codex if available (P6).
  ...
  **Gemini design subagent** (via Agent tool):
...
2. Step 0.5 (Dual Voices): Run Gemini subagent (foreground) first, then Codex. Present under
   CODEX SAYS (design — UX challenge) and GEMINI SUBAGENT (design — independent review)
   ...
   (not Gemini subagent — stays independent).
...
> **Phase 2 complete.** Codex: [N concerns]. Gemini subagent: [N issues].
...
- Dual voices: always run BOTH Gemini subagent AND Codex if available (P6).
  ...
  **Gemini eng subagent** (via Agent tool):
...
- Test plan: generate artifact at `~/.agy/projects/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
2. Step 0.5 (Dual Voices): Run Gemini subagent (foreground) first, then Codex. Present
   Codex output under CODEX SAYS (eng — architecture challenge) header. Present subagent
   output under GEMINI SUBAGENT (eng — independent review) header. Produce eng consensus
   table:
...
> **Phase 3 complete.** Codex: [N concerns]. Gemini subagent: [N issues].
> Consensus: [X/6 confirmed, Y disagreements → surfaced at gate].
...
- Dual voices: always run BOTH Gemini subagent AND Codex if available (P6).
  ...
  **Gemini DX subagent** (via Agent tool):
...
2. Step 0.5 (Dual Voices): Run Gemini subagent (foreground) first, then Codex. Present
   under CODEX SAYS (DX — developer experience challenge) and GEMINI SUBAGENT
...
> Codex: [N concerns]. Gemini subagent: [N issues].
> Consensus: [X/6 confirmed, Y disagreements → surfaced at gate].
...
- [ ] Dual voices ran (Codex + Gemini subagent, or noted unavailable)
- [ ] CEO consensus table produced
...
- [ ] Test plan artifact written to disk at ~/.agy/projects/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
TASKS_DIR="${HOME}/.gemini/projects/${SLUG:-unknown}"
...
### Review Scores
- CEO: [summary]
- CEO Voices: Codex [summary], Gemini subagent [summary], Consensus [X/6 confirmed]
- Design: [summary or "skipped, no UI scope"]
- Design Voices: Codex [summary], Gemini subagent [summary], Consensus [X/7 confirmed] (or "skipped")
- Eng: [summary]
- Eng Voices: Codex [summary], Gemini subagent [summary], Consensus [X/6 confirmed]
- DX: [summary or "skipped, no developer-facing scope"]
- DX Voices: Codex [summary], Gemini subagent [summary], Consensus [X/6 confirmed] (or "skipped")
...
agy-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

agy-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
agy-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
agy-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
agy-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

agy-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
agy-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
agy-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
