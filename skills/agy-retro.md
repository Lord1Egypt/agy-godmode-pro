...
Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using Gemini / agy as a force multiplier.
...
```bash
[ -f ~/.agy/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.agy/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat ~/.agy/greptile-history.md 2>/dev/null || true
...
# 12. agy skill usage telemetry (if available)
cat ~/.agy/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `~/.agy/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `~/.agy/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.agy/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
agy-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Note:** Only include the `greptile` field if `~/.agy/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
cat ~/.agy/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x agy-global-discover ] && DISCOVER_BIN=agy-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x ~/.gemini/skills/bin/agy-global-discover ] && DISCOVER_BIN=~/.gemini/skills/bin/agy-global-discover
[ -z "$DISCOVER_BIN" ] && which agy-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which agy-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/agy-global-discover.ts ] && DISCOVER_BIN="bun run bin/agy-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the agy directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/agy-discover-stderr
```

Read the stderr output from `/tmp/agy-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Behavioral patterns (e.g., "Codex used exclusively for myapp, agy / Gemini for everything else")
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: agy-global-discover" + "feat: /retro global template".
...
- agy / Gemini: N sessions across M repos — patterns observed
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.agy/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.agy/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.agy/retros/global-${today}-${next}.json`:
  ...
  "projects": [
    {
      ...
      "insertions": 3200,
      "deletions": 800,
      "sessions": { "gemini": 15, "codex": 3, "agy": 0 }
    }
  ],
  "totals": {
    "commits": 182,
    ...
    "projects": 5,
    "active_days": 6,
    "sessions": { "gemini": 48, "codex": 8, "agy": 3 },
    "global_streak_days": 52,
    "avg_context_switches_per_day": 2.1
  },
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | Gemini: 48, Codex: 8, agy: 3 | Focus: agy (58%) | Streak: 52d"
}
```
...
- Do not read GEMINI.md or other docs — this skill is self-contained
- On first run (no prior retros), skip comparison sections gracefully
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.agy/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".