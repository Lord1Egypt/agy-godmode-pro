...
_CEO=$(ls -t ~/.agy/projects/$SLUG/ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t ~/.agy/projects/$SLUG/designs/*/approved.json 2>/dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t ~/.agy/projects/$SLUG/designs/*/variant-*.png 2>/dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t ~/.agy/projects/$SLUG/designs/*/finalized.html 2>/dev/null | head -1)
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/~/.gemini/skills/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/~/.gemini/skills/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f ~/~/.gemini/skills/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/~/.gemini/skills/design-html/vendor/pretext.js
...
`~/.agy/projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`~/.agy/projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`
...
$B screenshot /tmp/agy-verify-mobile.png --width 375
$B screenshot /tmp/agy-verify-tablet.png --width 768
$B screenshot /tmp/agy-verify-desktop.png --width 1440
```
