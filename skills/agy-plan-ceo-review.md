...
```
Then read GEMINI.md, TODOS.md, and any existing architecture docs.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.agy/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$(agy-slug 2>/dev/null)" && mkdir -p ~/.agy/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.agy/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.agy/projects/$SLUG/ceo-plans/{old-plan}.md ~/.agy/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.agy/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.agy/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + agy,
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read `~/~/.gemini/skills/plan-ceo-review/sections/review-sections.md` and execute it
