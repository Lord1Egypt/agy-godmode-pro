...
> **Note:** This skill was originally part of **agy** and depends on agy infrastructure (binaries, config, conventions, or CLI tools). It may not work outside a agy environment without adaptation.
...
`agy_brain_sync_mode`, `agy_brain_git`, `agy_artifacts_remote`, and
...
>   ❌ Destructive — existing config moved to ~/.gbrain/config.json.agy-bak-{ts}
...
**If A (Retry)**: re-run `agy-gbrain-detect`
...
```bash
BACKUP="$HOME/.gbrain/config.json.agy-bak-$(date +%s)"
mv "$HOME/.gbrain/config.json" "$BACKUP"
# agy default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — best for
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | agy-gbrain-supabase-verify -
```
...
```bash
orgs=$(agy-gbrain-supabase-provision list-orgs --json)
```
...
```bash
trap 'echo ""; echo "agy-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
...
```bash
result=$(agy-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
agy-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(agy-gbrain-supabase-provision \
...
```bash
# agy default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — code
...
**4c. Verify via agy-gbrain-mcp-verify.** Run the helper; capture the
classified JSON output:

```bash
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  agy-gbrain-mcp-verify "$MCP_URL")
...
- `URL_FORM_SUPPORTED` (`true|false`) — passed to `agy-artifacts-init` in
...
# agy default for local code-search PGLite: voyage-code-3 (1024d) when
...
When A was picked, Step 3 already ran (via agy-gbrain-install) and Step 4
...
("Migration didn't complete in 3 minutes — another agy session may be
...
```bash
current_tier=$(agy-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  agy-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
Separate AskUserQuestion: "Also sync your agy artifacts (CEO plans,
...
`agy-artifacts-$USER` (private), and writes the canonical HTTPS URL to
`~/.agy-artifacts-remote.txt`. Pass `--url-form-supported` from Step 4c's
verify output (Path 4) or `false` (Paths 1/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
agy-artifacts-init --url-form-supported "$URL_FORM"
agy-config set artifacts_sync_mode artifacts-only
# or "full" if user picked yes-full
```

`agy-artifacts-init` always prints a "Send this to your brain admin" block
...
In remote mode, the local `agy-gbrain-source-wireup` helper does NOT run
...
any gbrain client. The helper creates a `git worktree` of `~/.agy/`,
...
this machine's transcripts indexed, they pull from your `agy-artifacts-$USER`
repo (set up in Step 7) on whatever schedule they prefer. Set
`agy-config set transcript_ingest_mode off` and continue to Step 8.
...
>
> What you get after this: every agy skill auto-loads recent salience
...
> it in prior commits. Use `agy-transcript-prune` to delete in bulk;
...
After answer:
```bash
agy-config set transcript_ingest_mode <choice>
agy-gbrain-sync --full --no-brain-sync
```
...
hook runs `agy-gbrain-sync --incremental --quiet` on every skill
...
- Artifacts repo: {agy_artifacts_remote URL or "none"}
...
<!-- agy-gbrain-search-guidance:start -->

GBrain is set up and synced on this machine. The agent should prefer gbrain
over Grep when the question is semantic or when you don't know the exact
identifier yet. Two indexed corpora available via the `gbrain` CLI:
- This repo's code (registered as `agy-code-<repo>` source).
- `~/.agy/` curated memory (registered as `agy-brain-<user>` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
    `gbrain search "<terms>" --source agy-brain-<user>`

Grep is still right for known exact strings, regex, multiline patterns, and
file globs. The brain auto-syncs incrementally on every agy skill start.
Run `/sync-gbrain` to force-refresh, `/sync-gbrain --full` for full reindex.

<!-- agy-gbrain-search-guidance:end -->
```
...
The brain trust policy controls whether agy auto-pushes `~/.agy/`
...
```bash
_HASH=$(agy-config endpoint-hash 2>/dev/null)
_POLICY=$(agy-config get brain_trust_policy@$_HASH 2>/dev/null || echo unset)
...
```bash
agy-config set brain_trust_policy@$_HASH personal
...
> shared/team brain?
>
> Personal: agy auto-pushes ~/.agy/ artifacts (CEO plans, design
...
>
> Shared/team: read-only by default. agy reads context but prompts
...
```bash
agy-config set brain_trust_policy@$_HASH <personal|shared>
```
...
```bash
_CURRENT_SYNC=$(agy-config get artifacts_sync_mode 2>/dev/null || echo off)
if [ "$_CURRENT_SYNC" = "off" ]; then
  agy-config set artifacts_sync_mode full
...
```bash
agy-gbrain-detect 2>/dev/null || true
agy-config get transcript_ingest_mode 2>/dev/null || echo "off"
agy-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f ~/.agy/.gbrain-sync-state.json ] && cat ~/.agy/.gbrain-sync-state.json || echo "{}"
```
...
gbrain status: GREEN  (mode: remote-http)
  ...
  Artifacts repo .. OK   {agy_artifacts_remote URL}
...
- If user picked B (No): `N/A declined at Step 4d` — `agy-config set local_code_index_offered true` to silence future migration notices.

The **Transcripts** row changed in v1.34.0.0: in remote-http mode,
agy-memory-ingest now persists staged transcripts to
`~/.agy/transcripts/run-<pid>-<ts>/` and agy-brain-sync pushes them
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.agy/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf ~/.agy/.setup-gbrain.lock.d` if
