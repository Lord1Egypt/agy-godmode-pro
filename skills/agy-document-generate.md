...
```bash
find . -type f -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.agy/*" -not -path "./dist/*" -not -path "./build/*" -not -path "./.next/*" | head -200
```
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  agy-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
