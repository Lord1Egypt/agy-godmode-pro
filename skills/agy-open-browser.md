...
Use when asked to "open agy browser", "launch browser", "connect chrome",
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.agy/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.agy/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.agy/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR=".agy/chromium-profile"
...
- The agy sidebar extension auto-loaded via `launchPersistentContext`
...
The `connect` command auto-discovers the extension from the agy install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.agy/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/~/.gemini/skills/extension/manifest.json" ] && _EXT_PATH="$_ROOT/~/.gemini/skills/extension"
[ -z "$_EXT_PATH" ] && [ -f "~/.gemini/skills/extension/manifest.json" ] && _EXT_PATH="~/.gemini/skills/extension"
...
> Chrome is launched with agy control. You should see Playwright's Chromium
...
>    already show the agy icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **agy browse** → click the **pin icon**
> 3. Click the pinned **agy icon** in the toolbar
...
> 2. Look for **"agy browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the agy icon
...
> - Run any agy skill (`/qa`, `/design-review`, `/benchmark`) and watch
