...
```
Scale must be 1-3 (agy policy cap). Changing `--scale` recreates the browser context; refs from `snapshot` are invalidated (rerun `snapshot`), but `load-html` content is replayed automatically. Not supported in headed mode.
...
`$B` is the browse binary (resolved from `$_ROOT/~/.gemini/skills/browse/dist/browse` or `~/~/.gemini/skills/browse/dist/browse`).
