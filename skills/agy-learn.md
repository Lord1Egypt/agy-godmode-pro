...
Review, search, prune, and export what agy
...
agy-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
agy will automatically capture patterns, pitfalls, and insights it discovers."
...
```bash
eval "$(agy-slug 2>/dev/null)"
agy-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(agy-slug 2>/dev/null)"
agy-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(agy-slug 2>/dev/null)"
agy-learnings-search --limit 50 2>/dev/null
```
...
Present the formatted output to the user. Ask if they want to append it to GEMINI.md
...
eval "$(agy-slug 2>/dev/null)"
eval "$(agy-paths)"
LEARN_FILE=".agy/state/projects/$SLUG/learnings.jsonl"
...
```bash
agy-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```