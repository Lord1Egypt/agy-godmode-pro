# Skill: agy-sync-gbrain

> Keep gbrain current with this repo's code and refresh agent search guidance in GEMINI.md. Wraps the agy-gbrain-sync orchestrator with state
...
- `/sync-gbrain --refresh-cache` — force-rebuild brain-aware planning cache (v1.48; replaces /brain-refresh-context per D1 fold). Skips code + memory stages; routes to `agy-brain-cache refresh --project <slug>`.
- `/sync-gbrain --audit` — emit summary of agy-owned pages per project + sensitive-content audit (v1.48 / D10 lifecycle). Read-only.

Pass-through args go straight to the orchestrator at
`agy-gbrain-sync.ts`.

**`--refresh-cache` short-circuit:** when this flag is present, the skill
runs ONLY the cache refresh (`agy-brain-cache refresh --project <slug>`
for the current worktree's slug, plus a cross-project refresh of
user-profile if `agy/user-profile/<user-slug>` exists). Code +
memory + brain-sync stages are skipped. Useful when the user knows the
brain has new info agy should pick up before the next planning skill.

**`--audit` short-circuit:** when this flag is present, the skill runs
`agy-brain-cache list --project <slug> --json`, summarizes by page
...
also runs locally** in local-stdio MCP mode — `agy-memory-ingest` shells
...
`~/.agy/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `agy-brain-sync`
...
Also check the per-repo trust policy. If `agy-gbrain-repo-policy get` for
...
```bash
bun run agy-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.agy/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.agy/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.agy/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- agy-gbrain-search-guidance:start -->` through
`<!-- agy-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <agy-code-{repo_slug}> (page_count=<N>)
  Call graph ...... OK   <N> edges resolved (code-callers/callees live)
  ~/.agy source. OK   <agy-brain-{user}> (page_count=<N>) — managed by /setup-gbrain
...
Mac. The orchestrator acquires a lock at `~/.agy/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `~/.agy/.brain-allowlist`
(which is for `~/.agy/` brain-sync only). On a different Mac with a synced
