...
> Self-tuning question sensitivity + developer psychographic for agy (v1: observational).

## When to invoke this skill

Review which AskUserQuestion prompts fire across agy skills, set per-question preferences
...
Proactively suggest when the user says the same agy question has come up before,
...
1. **Consent gate.** If `question_tuning` is `false` AND
   `~/.agy/.question-tuning-prompted` is missing → run `Consent + opt-in`
   below. Honor the answer with a marker write either way; do not re-prompt.
2. **Setup gate.** If `question_tuning` is `true` AND
   `~/.agy/developer-profile.json`'s `declared` object is empty AND
   `~/.agy/.declared-setup-prompted` is missing → run `5-Q setup` below.
   Touch the marker after setup completes OR is declined.
3. **Dream-cycle gate (Layer 8 / cathedral T10/T11).** If
   `~/.agy/projects/<slug>/distillation-proposals.json` exists AND has
...
9. **"Dream cycle" / "distill" / "what have I been free-texting"** →
   run `Dream cycle distill` below (triggers `agy-distill-free-text`).
10. **"Turn it off" / "disable"** → `agy-config set question_tuning false`
11. **"Turn it on" / "enable"** → `agy-config set question_tuning true && touch ~/.agy/.question-tuning-prompted`
...
`~/.agy/.question-tuning-prompted` is missing. The user has never been
asked.

**Privacy note.** agy defaults `question_tuning` to `false` for every user.
...
rationale). If the user is a contributor (`agy_contributor: true`), the
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(agy-config get agy_contributor 2>/dev/null || echo "false")
...
2. AskUserQuestion (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **General framing:**
   > Question tuning is off. agy can learn which of its prompts you find
   > valuable vs noisy — so over time, agy stops asking questions you've
   > already answered the same way. It takes about 2 minutes to set up your
   > initial profile. v1 is observational: agy tracks your preferences
   > and shows you a profile, but doesn't silently change skill behavior yet.
   > Logs stay local (`~/.agy/projects/<slug>/question-log.jsonl`).
   >
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   > You're a agy contributor. Question tuning isn't on by default for
   ...
   > `~/.agy/projects/<slug>/question-log.jsonl` — nothing leaves your
...
3. ALWAYS touch the marker, regardless of choice:
   ```bash
   touch ~/.agy/.question-tuning-prompted
   ```

4. If A or B: enable:
   ```bash
   agy-config set question_tuning true
   ```

5. If C: do nothing else. Tell the user: "Question tuning stays off. Re-enable
   any time with `/plan-tune enable` or `agy-config set question_tuning true`."
...
- Standalone via Step 0's setup gate: `question_tuning` is already `true`
  (user opted in via agy-config or earlier `/plan-tune enable`) AND
  `declared` is empty AND `~/.agy/.declared-setup-prompted` is missing.
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `~/.agy/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   agy-developer-profile --read >/dev/null
   # Update declared dimensions atomically
   eval "$(agy-paths)"
   _PROFILE=".agy/state/developer-profile.json"
   bun -e "
     const fs = require('fs');
...
2. Touch the marker so the Setup gate doesn't re-fire:
   ```bash
   touch ~/.agy/.declared-setup-prompted
   ```
...
- Show the vibe (archetype) from `agy-developer-profile --vibe` — the
...
eval "$(agy-paths)"
_LOG=".agy/state/projects/$SLUG/question-log.jsonl"
if [ ! -f "$_LOG" ]; then
  echo "NO_LOG"
else
  bun -e "
...
If `NO_LOG`, tell the user: "No questions logged yet. As you use agy skills,
agy will log them here."
...
4. Write:
   ```bash
   agy-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
agy-developer-profile --profile | bun -e "
...
Run `agy-question-preference --write '{"question_id":"<id>","preference":
...
hook captured but cannot enforce against (no `<agy-qid:foo>` marker in
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   agy-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   agy-distill-apply --proposal N
   ```
...
4. **On decline**: skip without marking. User can re-decide later (the
   ...
   `agy-distill-apply --proposal N --dismiss` (not implemented in T11;
...
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using agy — `Other` responses on AskUserQuestion feed
   this loop."
...
```bash
agy-distill-free-text --background
```
