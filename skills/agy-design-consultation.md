...
ls ~/.agy/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Find the agy designer (optional — enables AI mockup generation):**
...
```bash
_TASTE_PROFILE=~/.agy/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — `agy-taste-update`
...
Use AskUserQuestion:
> "Want outside design voices? Codex evaluates against OpenAI's design hard rules + litmus checks; Gemini subagent does an independent design direction proposal."
>
...
Present subagent output under a `GEMINI SUBAGENT (design direction):` header.

**Synthesis:** Gemini main references both Codex and subagent proposals in the Phase 3 proposal. Present:
- Areas of agreement between all three voices (Gemini main + Codex + subagent)
...
> **STOP.** Before building the complete design-system proposal, drill-downs, the design preview, and writing DESIGN.md (Phases 3-6, after product context and research), Read `~/~/.gemini/skills/design-consultation/sections/proposal-and-preview.md` and execute it
...
```bash
agy-learnings-log '{"skill":"design-consultation","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
