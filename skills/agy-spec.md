...
>
> **Note:** This skill references agy-specific commands (`agy-config`, `agy-slug`, `agy-redact`, etc.) that must be available in your environment. Genericize or replace as needed.

## When to invoke this skill

Files the issue,
optionally spawns a Gemini agent in a fresh worktree, and lets /ship close
...
bun ~/~/.gemini/skills/lib/redact-audit-log.ts \
...
# Resolve visibility once; cache + reuse. Order: local config (~/.agy, never
# committed) → gh → glab → unknown(=public-strict).
REDACT_VIS=$(agy-config get redact_repo_visibility 2>/dev/null)
...
REDACT_BODY_EOF
REDACT_JSON=$(agy-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
Read `PLAN_MODE` from the environment (emitted by `## Preamble (run first)

```bash
_UPD=$(agy-update-check 2>/dev/null || ~/.gemini/skills/bin/agy-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p ~/.agy/sessions
touch ~/.agy/sessions/"$PPID"
_SESSIONS=$(find ~/.agy/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find ~/.agy/sessions -mmin +120 -type f -exec rm {} + 2>/dev/null || true
_PROACTIVE=$(agy-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.agy/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(agy-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(agy-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_SESSION_KIND=$(agy-session-kind 2>/dev/null || echo "interactive")
case "$_SESSION_KIND" in spawned|headless|interactive) ;; *) _SESSION_KIND="interactive" ;; esac
echo "SESSION_KIND: $_SESSION_KIND"
_LAKE_SEEN=$([ -f ~/.agy/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(agy-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f ~/.agy/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$(agy-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(agy-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
mkdir -p ~/.agy/analytics
if [ "$_TEL" != "off" ]; then
echo '{"skill":"spec","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(_repo=$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null | tr -cd 'a-zA-Z0-9._-'); echo "${_repo:-unknown}")'"}'  >> ~/.agy/analytics/skill-usage.jsonl 2>/dev/null || true
fi
for _PF in $(find ~/.agy/analytics -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "agy-telemetry-log" ]; then
      agy-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
    rm -f "$_PF" 2>/dev/null || true
  fi
  break
done
eval "$(agy-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="${PROJECT_HOME:-$HOME/.agy}/projects/${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    agy-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
agy-timeline-log '{"skill":"spec","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f GEMINI.md ] && grep -q "## Skill routing" GEMINI.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(agy-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d ".gemini/skills/agy" ] && [ ! -L ".gemini/skills/agy" ]; then
  if [ -f "~/.gemini/skills/VERSION" ] || [ -d "~/.gemini/skills/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_GSTACK: $_VENDORED"
echo "MODEL_OVERLAY: gemini"
_CHECKPOINT_MODE=$(agy-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(agy-config get checkpoint_push 2>/dev/null || echo "false")
echo "CHECKPOINT_MODE: $_CHECKPOINT_MODE"
echo "CHECKPOINT_PUSH: $_CHECKPOINT_PUSH"
# Plan-mode hint for skills like /spec that branch behavior on plan-mode state.
# Gemini exposes plan mode via system reminders; we detect best-effort
# from PLAN_FILE (set by the harness when plan mode is active) and
# fall back to "inactive". Codex hosts and Gemini execution mode both end up
# inactive, which is the safe default (defaults to file+execute pipeline).
if [ -n "${PLAN_FILE:-}${PLAN_MODE_FORCE:-}" ]; then
  export PLAN_MODE="active"
elif [ "${PLAN_MODE:-}" = "active" ]; then
  export PLAN_MODE="active"
else
  export PLAN_MODE="inactive"
fi
...
In plan mode, allowed because they inform the plan: `$B`, `$D`, `codex exec`/`codex review`, writes to `~/.agy/`, writes to the plan file, and `open` for generated artifacts.
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/agy-*` names. Disk paths stay `~/~/.gemini/skills/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/~/.gemini/skills/agy-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running agy v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `~/~/.gemini/skills/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `agy-config set checkpoint_mode continuous`. Always touch marker.
- Missing `~/~/.gemini/skills/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `agy-config set explain_level terse`.

Always run (regardless of choice):
```bash
rm -f ~/.agy/.writing-style-prompt-pending
touch ~/.agy/.writing-style-prompted
```

Skip if `WRITING_STYLE_PENDING` is `no`.

If `LAKE_INTRO` is `no`: say "agy follows the **Boil the Ocean** principle — do the complete thing when AI makes marginal cost near-zero. Read more: https://garryslist.org/posts/boil-the-ocean" Offer to open:

```bash
open https://garryslist.org/posts/boil-the-ocean
touch ~/.agy/.completeness-intro-seen
```
...
> Help agy get better. Share usage data only: skill, duration, crashes, stable device ID. No code or file paths. Your repo name is recorded locally only and stripped before any upload.

Options:
- A) Help agy get better! (recommended)
- B) No thanks

If A: run `agy-config set telemetry community`
...
If B→A: run `agy-config set telemetry anonymous`
If B→B: run `agy-config set telemetry off`

Always run:
```bash
touch ~/.agy/.telemetry-prompted
```
...
> Let agy proactively suggest skills, like /qa for "does this work?" or /investigate for bugs?

Options:
- A) Keep it on (recommended)
- B) Turn it off — I'll type /commands myself

If A: run `agy-config set proactive true`
If B: run `agy-config set proactive false`

Always run:
```bash
touch ~/.agy/.proactive-prompted
```
...
> agy works best when your project's GEMINI.md includes skill routing rules.
...
Then commit the change: `git add GEMINI.md && git commit -m "chore: add agy skill routing rules to GEMINI.md"`

If B: run `agy-config set routing_declined true` and say they can re-enable with `agy-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_GSTACK` is `yes`, warn once via AskUserQuestion unless `~/.agy/.vendoring-warned-$SLUG` exists:

> This project has agy vendored in `~/.gemini/skills/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r ~/.gemini/skills/`
2. Run `echo '~/.gemini/skills/' >> .gitignore`
3. Run `agy-team-init required` (or `optional`)
4. Run `git add .gemini/ .gitignore GEMINI.md && git commit -m "chore: migrate agy from vendored to team mode"`
5. Tell the user: "Done. Each developer now runs: `cd ~/.gemini/skills/agy && ./setup --team`"
...
```bash
eval "$(agy-slug 2>/dev/null)" 2>/dev/null || true
touch ~/.agy/.vendoring-warned-${SLUG:-unknown}
```
...
"AskUserQuestion" can resolve to two tools at runtime: the **host MCP variant** (e.g. `mcp__conductor__AskUserQuestion` — appears in your tool list when the host registers it) or the **native** Gemini tool.
...
Effort both-scales: when an option involves effort, label both human-team and CC+agy time, e.g. `(human: ~2 days / CC: ~15 min)`. Makes AI compression visible at decision time.
...
(`bin/agy-question-preference`) refuses `never-ask` on any `*-split-*` id,
so split chains are never AUTO_DECIDE-eligible — the user's option set is sacred.

**Full rule + worked examples + Hold/dependency semantics:** see
`docs/askuserquestion-split.md` in the agy repo. Read on demand when N>4.
...
```bash
_PROJECT_HOME="${PROJECT_HOME:-$HOME/.agy}"
# Prefer the v1.27.0.0 artifacts file; fall back to brain file for users
# upgrading mid-stream before the migration script runs.
if [ -f "$HOME/.agy-artifacts-remote.txt" ]; then
  _BRAIN_REMOTE_FILE="$HOME/.agy-artifacts-remote.txt"
else
  _BRAIN_REMOTE_FILE="$HOME/.agy-brain-remote.txt"
fi
_BRAIN_SYNC_BIN="agy-brain-sync"
_BRAIN_CONFIG_BIN="agy-config"
...
if [ -f "$_GBRAIN_CONFIG" ] && command -v gbrain >/dev/null 2>&1; then
  ...
  if [ "$_GBRAIN_VERSION_OK" -gt 0 ] 2>/dev/null; then
    ...
    if [ -n "$_GBRAIN_PIN_PATH" ]; then
      ...
      echo "symbol-aware code lookup. See \"## GBrain Search Guidance\" in GEMINI.md."
...
# own cadence. Read gemini.json directly to keep this preamble fast (no
# subprocess to agy CLI on every skill start).
_GBRAIN_MCP_MODE="none"
if command -v jq >/dev/null 2>&1 && [ -f "./.gemini.json" ]; then
  _GBRAIN_MCP_TYPE=$(jq -r '.mcpServers.gbrain.type // .mcpServers.gbrain.transport // empty' "./.gemini.json" 2>/dev/null)
...
if [ -f "$_BRAIN_REMOTE_FILE" ] && [ ! -d "$_PROJECT_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" = "off" ]; then
  _BRAIN_NEW_URL=$(head -1 "$_BRAIN_REMOTE_FILE" 2>/dev/null | tr -d '[:space:]')
  if [ -n "$_BRAIN_NEW_URL" ]; then
    echo "ARTIFACTS_SYNC: artifacts repo detected: $_BRAIN_NEW_URL"
    echo "ARTIFACTS_SYNC: run 'agy-brain-restore' to pull your cross-machine artifacts (or 'agy-config set artifacts_sync_mode off' to dismiss forever)"
  fi
fi

if [ -d "$_PROJECT_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" != "off" ]; then
  _BRAIN_LAST_PULL_FILE="$_PROJECT_HOME/.brain-last-pull"
  _BRAIN_NOW=$(date +%s)
  _BRAIN_DO_PULL=1
  ...
  if [ "$_BRAIN_DO_PULL" = "1" ]; then
    ( cd "$_PROJECT_HOME" && git fetch origin >/dev/null 2>&1 && git merge --ff-only "origin/$(git rev-parse --abbrev-ref HEAD)" >/dev/null 2>&1 ) || true
...
if [ "$_GBRAIN_MCP_MODE" = "remote-http" ]; then
  ...
  _GBRAIN_HOST=$(jq -r '.mcpServers.gbrain.url // empty' "./.gemini.json" 2>/dev/null | sed -E 's|^https?://([^/:]+).*|\1|')
  echo "ARTIFACTS_SYNC: remote-mode (managed by brain server ${_GBRAIN_HOST:-remote})"
elif [ -d "$_PROJECT_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" != "off" ]; then
  _BRAIN_QUEUE_DEPTH=0
  [ -f "$_PROJECT_HOME/.brain-queue.jsonl" ] && _BRAIN_QUEUE_DEPTH=$(wc -l < "$_PROJECT_HOME/.brain-queue.jsonl" | tr -d ' ')
  _BRAIN_LAST_PUSH="never"
  [ -f "$_PROJECT_HOME/.brain-last-push" ] && _BRAIN_LAST_PUSH=$(cat "$_PROJECT_HOME/.brain-last-push" 2>/dev/null || echo never)
...
> agy can publish your artifacts (CEO plans, designs, reports) to a private GitHub repo that GBrain indexes across machines. How much should sync?
...
If A/B and `~/.agy/.git` is missing, ask whether to run `agy-artifacts-init`. Do not block the skill.

At skill END before telemetry:

```bash
"agy-brain-sync" --discover-new 2>/dev/null || true
"agy-brain-sync" --once 2>/dev/null || true
```
...
```bash
eval "$(agy-slug 2>/dev/null)"
_PROJ="${PROJECT_HOME:-$HOME/.agy}/projects/${SLUG:-unknown}"
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    agy-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
**Cross-session decisions.** If `ACTIVE DECISIONS` are listed, treat them as prior settled calls with their rationale — do not silently re-litigate them; if you're about to reverse one, say so explicitly. Reach for `agy-decision-search` whenever a question touches a past decision ("what did we decide / why / did we try"). When you or the user make a DURABLE decision (architecture, scope, tool/vendor choice, or a reversal) — NOT a turn-level or trivial choice — log it with `agy-decision-log` (`--supersede <id>` for a reversal). Reliable and local; gbrain not required.
...
Curated jargon list lives at `~/~/.gemini/skills/scripts/jargon-list.json` (80+ terms). On the first jargon term you encounter this session, Read that file once; treat the `terms` array as the canonical list. The list is repo-owned and may grow between releases.
...
`/context-restore` reads `[agy-context]`; `/ship` squashes WIP commits into clean commits.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `agy-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.

**Embed the question_id as a marker in the question text** so hooks can identify it deterministically (plan-tune cathedral T14 / D18 progressive markers). Append `<agy-qid:{question_id}>` somewhere in the rendered question (the leading line or trailing line is fine; the marker doesn't render visibly to the user when wrapped in HTML-style angle brackets, but the hook strips it). Without the marker the PreToolUse enforcement hook treats the AUQ as observed-only and never auto-decides — so always include it when the question matches a registered `question_id`.

**Embed the option recommendation via the `(recommended)` label suffix** on exactly one option per AUQ. The PreToolUse hook parses `(recommended)` first, falls back to "Recommendation: X" prose, and refuses to auto-decide if ambiguous. Two `(recommended)` labels = refuse.

After answer, log best-effort (PostToolUse hook also captures deterministically when installed; dedup on (source, tool_use_id) handles double-writes):
```bash
agy-question-log '{"skill":"spec","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
agy-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `~/~/.gemini/skills/ETHOS.md`.
- **Layer 1** (tried and true) — don't reinvent. **Layer 2** (new and popular) — scrutinize. **Layer 3** (first principles) — prize above all.

**Eureka:** When first-principles reasoning contradicts conventional wisdom, name it and log:
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.agy/analytics/eureka.jsonl 2>/dev/null || true
```
...
```bash
agy-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
`~/.agy/analytics/`, matching preamble analytics writes.
...
rm -f ~/.agy/analytics/.pending-"$_SESSION_ID" 2>/dev/null || true
# Session timeline: record skill completion (local-only, never sent anywhere)
agy-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> ~/.agy/analytics/skill-usage.jsonl 2>/dev/null || true
fi
