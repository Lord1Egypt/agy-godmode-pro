...
```bash
eval "$(agy-slug 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .agy/canary-reports
mkdir -p .agy/canary-reports/baselines
mkdir -p .agy/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".agy/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.agy/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".agy/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".agy/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
Screenshots:   .agy/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.agy/canary-reports/{date}-canary.md` and `.agy/canary-reports/{date}-canary.json`.
