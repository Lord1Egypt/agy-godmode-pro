...
if [ ! -f ~/.agy/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.agy/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
mkdir -p ~/.agy/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' GEMINI.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.agy/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
```bash
cat ~/~/.gemini/skills/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
ls -t ~/.agy-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.agy-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `agy-diff-scope` to classify the changes:

```bash
eval $(agy-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".agy/deploy-reports/post-deploy.png"
```
...
Save report to `.agy/deploy-reports/{date}-pr{number}-deploy.md`.
