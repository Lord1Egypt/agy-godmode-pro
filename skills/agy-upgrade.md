...
> Upgrade agy to the latest version.
...
runs the upgrade, and shows what's new. Use when asked to "upgrade agy",
"update agy", or "get latest version".

Voice triggers (speech-to-text aliases): "upgrade the tools", "update the tools", "gee stack upgrade", "g stack upgrade".

# /agy-upgrade

Upgrade agy to the latest version and show what's new.
...
[ -z "$_AUTO" ] && _AUTO=$(agy-config get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip AskUserQuestion. Log "Auto-upgrading agy v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/agy-upgrade` manually to retry."

**Otherwise**, use AskUserQuestion:
- Question: "agy **v{new}** is available (you're on v{old}). Upgrade now?"
...
```bash
agy-config set auto_upgrade true
```
Tell user: "Auto-upgrade enabled. Future updates will install automatically." Then proceed to Step 2.

**If "Not now":** Write snooze state with escalating backoff (first snooze = 24h, second = 48h, third+ = 1 week), then continue with the current skill. Do not mention the upgrade again.
```bash
_SNOOZE_FILE=".agy/update-snoozed"
_REMOTE_VER="{new}"
_CUR_LEVEL=0
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week, depending on level). Tip: "Set `auto_upgrade: true` in `~/.agy/config.yaml` for automatic upgrades."

**If "Never ask again":**
```bash
agy-config set update_check false
```
Tell user: "Update checks disabled. Run `agy-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "~/.gemini/skills/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR=".gemini/skills/agy"
elif [ -d ".agy/repos/agy/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR=".agy/repos/agy"
elif [ -d "~/.gemini/skills/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".gemini/skills/agy"
elif [ -d ".agents/skills/agy/.git" ]; then
  INSTALL_TYPE="local-git"
  INSTALL_DIR=".agents/skills/agy"
elif [ -d ".gemini/skills/agy" ]; then
...
TMP_DIR=$(mktemp -d)
git clone --depth 1 https://github.com/garrytan/agy.git "$TMP_DIR/agy"
mv "$INSTALL_DIR" "$INSTALL_DIR.bak"
mv "$TMP_DIR/agy" "$INSTALL_DIR"
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.gemini/skills/agy" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/.gemini/skills/agy" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/.gemini/skills/agy"
  fi
fi
_TEAM_MODE=$(agy-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached ~/.gemini/skills/ 2>/dev/null || true
if ! grep -qF '~/.gemini/skills/' .gitignore 2>/dev/null; then
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `~/.gemini/skills/` when you're ready."
...
```
Tell user: "Sync failed — restored previous version at `$LOCAL_GSTACK`. Run `/agy-upgrade` manually to retry."
...
```bash
MIGRATIONS_DIR="$INSTALL_DIR/agy-upgrade/migrations"
...
Migrations are idempotent bash scripts in `agy-upgrade/migrations/`. Each is named
...
```bash
mkdir -p ~/.agy
echo "$OLD_VERSION" > ~/.agy/just-upgraded-from
rm -f ~/.agy/last-update-check
rm -f ~/.agy/update-snoozed
```
...
When invoked directly as `/agy-upgrade` (not from a preamble):

1. Force a fresh update check (bypass cache):
```bash
agy-update-check --force 2>/dev/null || \
~/.gemini/skills/bin/agy-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `~/.gemini/skills/` when you're ready."
