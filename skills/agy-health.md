...
queue_component:   10 if ~/.agy/.brain-queue.jsonl has <10 lines;
                    7 if 10-100; 0 if >=100 (suggests secret-scan rejections
                    piling up). N/A if artifacts_sync_mode == off.
push_component:    10 if (now - mtime of ~/.agy/.brain-last-push) < 24h;
...
Append one JSONL line to `~/.agy/projects/$SLUG/health-history.jsonl`:
...
Read the last 10 entries from `~/.agy/projects/$SLUG/health-history.jsonl` (if the
file exists and has prior entries).

```bash
eval "$(agy-slug 2>/dev/null)" && mkdir -p ~/.agy/projects/$SLUG
tail -10 ~/.agy/projects/$SLUG/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```
