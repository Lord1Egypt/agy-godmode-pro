...
```bash
cd "$_ROOT"
git rm -r --cached .gemini/skills/gstack/ 2>/dev/null || true
if ! grep -qF '.gemini/skills/gstack/' .gitignore 2>/dev/null; then
  echo '.gemini/skills/gstack/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
