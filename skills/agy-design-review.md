...
[ -f .agy/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.agy/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.agy/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.agy/no-test-bootstrap` and re-run." Continue without tests.
...
**Find the agy designer (optional — enables target mockup generation):**
...
REPORT_DIR=".agy/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)"
...
**Local:** `.agy/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(agy-slug 2>/dev/null)" && mkdir -p ~/.agy/projects/$SLUG
```
Write to: `~/.agy/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + agy design methodology.
...
```
~/.agy/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
Present subagent output under a `GEMINI SUBAGENT (design consistency):` header.
...
If the agy designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
eval "$(agy-slug 2>/dev/null)" && mkdir -p ~/.agy/projects/$SLUG
```
Write a one-line summary to `~/.agy/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
agy-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
