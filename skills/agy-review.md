...
>
> **Note:** This skill references agy-specific commands (`agy-config`, `agy-review-log`, etc.) that must be available in your environment. Genericize or replace as needed.
...
# Compute project slug for ~/.agy/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in ".agy/projects/$_PLAN_SLUG" ".gemini/plans" "$HOME/.codex/plans" ".agy/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## AGY REVIEW REPORT`)
...
QUEUE_JSON=$(bun run bin/agy-next-version \
  --base "$BASE_BRANCH" \
  --bump patch \
...
`~/.agy-dev/plans/1539-framework-aware-review.md` design doc.
...
```bash
source <(agy-diff-scope <base> 2>/dev/null) || true
...
```bash
agy-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/~/.gemini/skills/review/specialists/testing.md`
2. **Maintainability** — read `~/~/.gemini/skills/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/~/.gemini/skills/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/~/.gemini/skills/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/~/.gemini/skills/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/~/.gemini/skills/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/~/.gemini/skills/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `agy-specialist-stats` output above:
...
```bash
agy-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/~/.gemini/skills/review/specialists/red-team.md`
...
Cross-reference the diff against documentation files. For each `.md` file in the repo root (README.md, ARCHITECTURE.md, CONTRIBUTING.md, GEMINI.md, etc.):
...
Every diff gets adversarial review from both Gemini and Codex. LOC is not a proxy for risk — a 5-line auth change can be critical.
...
if [ "$_CODEX_CFG" = "disabled" ]; then
  ...
  _CODEX_MODE="not_installed"; _agy_codex_log_event "codex_cli_missing" 2>/dev/null || true
elif ! _agy_codex_auth_probe >/dev/null 2>&1; then
  _CODEX_MODE="not_authed"; _agy_codex_log_event "codex_auth_failed" 2>/dev/null || true
else
  _CODEX_MODE="ready"; _agy_codex_version_check 2>/dev/null || true
fi
echo "CODEX_MODE: $_CODEX_MODE"
```

Branch on the echoed `CODEX_MODE`:
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip the Codex passes only; the Gemini adversarial subagent below STILL runs (it is free and fast). Print: "Codex passes skipped (codex_reviews disabled) — running Gemini adversarial only."
- **`not_installed`** — Codex CLI absent. Print: "Codex not installed — using Gemini subagent. Install for cross-model coverage: `npm install -g @openai/codex`." Fall back to the Gemini subagent path.
- **`not_authed`** — installed but no credentials. Print: "Codex installed but not authenticated — using Gemini subagent. Run `codex login` or set `$CODEX_API_KEY`." Fall back to the Gemini subagent path.
- **`ready`** — run the Codex pass below.

For this diff-review path, `CODEX_MODE: disabled` means skip the Codex passes ONLY — the
Gemini adversarial subagent below still runs (it's free and fast). `ready` runs the Codex
...
### Gemini adversarial subagent (always runs)
...
Present findings under an `ADVERSARIAL REVIEW (Gemini subagent):` header. **FIXABLE findings** flow into the same Fix-First pipeline as the structured review. **INVESTIGATE findings** are presented as informational.

If the subagent fails or times out: "Gemini adversarial subagent unavailable. Continuing."
...
If `CODEX_MODE` is `not_installed` / `not_authed` / `disabled`: the preflight already printed the reason; run Gemini adversarial only.
...
If `DIFF_TOTAL < 200`: skip this section silently. The Gemini + Codex adversarial passes provide sufficient coverage for smaller diffs.
...
```bash
agy-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
════════════════════════════════════════════════════════════
  ...
  Unique to Gemini structured review: [from earlier step]
  Unique to Gemini adversarial: [from subagent]
  Unique to Codex: [from codex adversarial or code review, if ran]
  Models used: Gemini structured ✓  Gemini adversarial ✓/✗  Codex ✓/✗
════════════════════════════════════════════════════════════
```
...
```bash
agy-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
