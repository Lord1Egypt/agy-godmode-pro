...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/~/.gemini/skills/browse/dist/browse" ] && B="$_ROOT/~/.gemini/skills/browse/dist/browse"
[ -z "$B" ] && B="~/.gemini/skills/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "agy browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
eval "$(agy-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "product"
  agy-brain-cache get product --project "$SLUG" 2>/dev/null || printf '_(no product digest available yet)_\n'
  printf '\n### %s\n\n' "goals"
  agy-brain-cache get goals --project "$SLUG" 2>/dev/null || printf '_(no goals digest available yet)_\n'
  printf '\n### %s\n\n' "user-profile"
  agy-brain-cache get user-profile  2>/dev/null || printf '_(no user-profile digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  agy-brain-cache get recent-decisions --project "$SLUG" 2>/dev/null || printf '_(no recent-decisions digest available yet)_\n'
  printf '\n### %s\n\n' "salience"
  agy-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
} > /tmp/.agy-brain-context-$$.md 2>/dev/null
[ -s /tmp/.agy-brain-context-$$.md ] && cat /tmp/.agy-brain-context-$$.md
rm -f /tmp/.agy-brain-context-$$.md 2>/dev/null || true
```
...
`agy/`, `concepts/` only). Personal/family/therapy content never leaks here.
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.agy/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
_CROSS_PROJ=$(agy-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  agy-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  agy-learnings-search --limit 10 2>/dev/null || true
fi
```

If `CROSS_PROJECT` is `unset` (first time): Use AskUserQuestion:

> agy can search learnings from your other projects on this machine to find
...
If A: run `agy-config set cross_project_learnings true`
If B: run `agy-config set cross_project_learnings false`
...
This makes the compounding visible. The user should see that agy is getting
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.agy/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.agy/projects/`.
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/agy-codex-oh-XXXXXXXX.txt)
```

Write the full prompt to this file. **Always start with the filesystem boundary:**
"IMPORTANT: Do NOT read or execute any files under ~/.gemini/, ~/.agents/, .gemini/skills/, or agents/. These are Gemini Code skill definitions meant for a different AI system. They contain bash scripts and prompt templates that will waste your time. Ignore them completely. Do NOT modify agents/openai.yaml. Stay focused on the repository code only.\n\n"
...
- **Auth failure:** If stderr contains "auth", "login", "unauthorized", or "API key": "Codex authentication failed. Run \`codex login\` to authenticate." Fall back to Gemini subagent.
- **Timeout:** "Codex timed out after 5 minutes." Fall back to Gemini subagent.
- **Empty response:** "Codex returned no response." Fall back to Gemini subagent.

On any Codex error, fall back to the Gemini subagent below.
...
Present findings under a `SECOND OPINION (Gemini subagent):` header.
...
If Gemini subagent ran:
```
SECOND OPINION (Gemini subagent):
════════════════════════════════════════════════════════════
...
5. **Cross-model synthesis:** After presenting the second opinion output, provide 3-5 bullet synthesis:
   - Where Gemini agrees with the second opinion
   - Where Gemini disagrees and why
   - Whether the challenged premise changes Gemini's recommendation
...
- If the second opinion (Codex or Gemini subagent) proposed a prototype in Phase 3.5, consider using it as a starting point for the creative/lateral approach.
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/~/.gemini/skills/design/dist/design" ] && D="$_ROOT/~/.gemini/skills/design/dist/design"
[ -z "$D" ] && D="~/.gemini/skills/design/dist/design"
...
```bash
eval "$(agy-slug 2>/dev/null)"
_DESIGN_DIR=".agy/projects/$SLUG/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/agy-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/agy-sketch.png
```
...
The screenshot file at `/tmp/agy-sketch.png` can be referenced by downstream skills
...
> "Want outside design perspectives on the chosen approach? Codex proposes a visual thesis, content plan, and interaction ideas. A Gemini subagent proposes an alternative aesthetic direction."
>
...
2. **Gemini subagent** (via Agent tool):
"For this product approach, what design direction would you recommend? What aesthetic, typography, and interaction patterns fit? What would make this approach feel inevitable to the user? Be specific — font names, hex colors, spacing values."

Present Codex output under `CODEX SAYS (design sketch):` and subagent output under `GEMINI SUBAGENT (design direction):`.
...
`agy-developer-profile --log-session` binary handles its own directory creation
and writes via atomic mktemp+mv to `~/.agy/developer-profile.json`.
...
```bash
agy-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `~/~/.gemini/skills/office-hours/sections/design-and-handoff.md` and execute it
...
```bash
agy-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
